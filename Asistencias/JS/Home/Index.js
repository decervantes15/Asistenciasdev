$(document).on('click', '#enter', function () {

    var aviso = $('#aviso');
    aviso.removeClass('text-success');
    aviso.addClass('text-danger');
    aviso.html('');

    var username = $('#user').val();
    var tipo_registro = $('#tipo_registro').val();
    var password = $('#password').val();

    if (username.length == 0) {
        aviso.html('<br /> Debe de ingresar el usuario');
    }
    else if (tipo_registro.length == 0) {
        aviso.html('<br /> Debe de seleccionar el tipo de registro');
    }
    else if (password.length == 0) {
        aviso.html('<br /> Debe de ingresar la contraseña');
    }
    else {
        AjaxPost('/api/v1/registrar',
            { username, tipo_registro, password },
            function (respuesta) {

                aviso.html('<br />' + respuesta.Mensaje);
                if (respuesta.Estatus == 1) {
                    aviso.addClass('text-success');
                    aviso.removeClass('text-danger');

                    $('#user').val('');
                    $('#tipo_registro').val('');
                    $('#password').val('');
                }
            }
        );
    }
});

$(document).on('click', '#asistencias', function () {

    var aviso = $('#aviso');
    aviso.removeClass('text-success');
    aviso.addClass('text-danger');
    aviso.html('');

    var username = $('#user').val();
    var password = $('#password').val();

    if (username.length == 0) {
        aviso.html('<br /> Debe de ingresar el usuario');
    }
    else if (password.length == 0) {
        aviso.html('<br /> Debe de ingresar la contraseña');
    }
    else {
        AjaxPost('/api/v1/login',
            { username, password },
            function (respuesta) {

                if (respuesta.Estatus == 1) {
                    window.location.href = "/Consulta/MisAsistencias";
                }
                else {
                    aviso.html('<br />' + respuesta.Mensaje);
                }
            }
        );
    }
});
